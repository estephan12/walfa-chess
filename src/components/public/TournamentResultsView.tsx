    ...
    <div className="space-y-12">
      ...
      {categories.length > 0 && (
        <div className="flex items-center gap-2 overflow-x-auto pb-2 border-b border-slate-200 scrollbar-none">
          <div className="flex items-center gap-1.5 text-xs text-slate-500 mr-2 font-bold uppercase tracking-wider shrink-0">
      ...
      {top3.length > 0 && (
        <section className="space-y-6">
          <div className="text-center space-y-1">
            <span className="text-xs font-black uppercase tracking-widest text-[#1D64F2]">
              Cuadro de Honor
            </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-end pt-6">
            {/* Segundo Lugar (Plata) */}
            {second ? (
              <div className="order-2 md:order-1 bg-white border border-slate-200 rounded-2xl p-6 text-center shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
                <div className="absolute top-0 inset-x-0 h-1.5 bg-slate-400" />
                <div className="flex items-center justify-center -mt-2 mb-3">
                  <div className="h-10 w-10 rounded-full bg-slate-100 border border-slate-300 flex items-center justify-center text-slate-700">
                ...
                <div className="relative h-16 w-16 mx-auto rounded-full overflow-hidden border-2 border-slate-300 bg-slate-100 my-3">
                ...
                {second.player?.club && (
                  <p className="text-xs text-slate-500">{second.player.club}</p>
                )}

                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-around text-xs">
                  {second.points !== null && (
                    <div>
                      <span className="text-slate-400 block text-[10px]">Puntos</span>
                      <span className="font-mono font-bold text-slate-900">{second.points} pts</span>
                    </div>
                  )}
                  {second.rating_performance && (
                    <div>
                      <span className="text-slate-400 block text-[10px]">Performance</span>
                      <span className="font-mono font-bold text-emerald-600">{second.rating_performance}</span>
                    </div>
                  )}
                </div>

                {second.prize_won && (
                  <p className="mt-3 text-xs font-bold text-slate-800 bg-slate-100 py-1.5 px-3 rounded-lg border border-slate-200">
                    {second.prize_won}
                  </p>
                )}
              </div>
            ...
            {first && (
              <div className="order-1 md:order-2 bg-white border-2 border-amber-400 rounded-3xl p-8 text-center shadow-md shadow-amber-500/10 relative overflow-hidden group md:-translate-y-4">
                <div className="absolute top-0 inset-x-0 h-2 bg-gradient-to-r from-amber-400 via-yellow-400 to-amber-500" />
                <div className="flex items-center justify-center -mt-2 mb-3">
                  <div className="h-12 w-12 rounded-full bg-amber-50 border-2 border-amber-400 flex items-center justify-center text-amber-600 shadow-sm">
                ...
                <div className="relative h-20 w-20 mx-auto rounded-full overflow-hidden border-2 border-amber-400 bg-amber-50 my-3 shadow-sm">
                ...
                {first.player?.club && (
                  <p className="text-xs text-slate-500">{first.player.club}</p>
                )}
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-around text-xs">
                  {first.points !== null && (
                    <div>
                      <span className="text-slate-400 block text-[10px]">Puntos</span>
                      <span className="font-mono font-bold text-amber-600 text-sm">{first.points} pts</span>
                    </div>
                  )}
                  {first.rating_performance && (
                    <div>
                      <span className="text-slate-400 block text-[10px]">Performance</span>
                      <span className="font-mono font-bold text-emerald-600 text-sm">{first.rating_performance}</span>
                    </div>
                  )}
                </div>

                {first.prize_won && (
                  <p className="mt-3 text-xs font-bold text-amber-800 bg-amber-50 py-2 px-3 rounded-xl border border-amber-200">
                    🏆 {first.prize_won}
                  </p>
                )}
              </div>
            ...
            {/* Tercer Lugar (Bronce) */}
            {third ? (
              <div className="order-3 bg-white border border-orange-200 rounded-2xl p-6 text-center shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
                <div className="absolute top-0 inset-x-0 h-1.5 bg-orange-500" />
                <div className="flex items-center justify-center -mt-2 mb-3">
                  <div className="h-10 w-10 rounded-full bg-orange-50 border border-orange-300 flex items-center justify-center text-orange-600">
                    <Medal className="h-5 w-5" />
                  </div>
                </div>
                ...
                <div className="relative h-16 w-16 mx-auto rounded-full overflow-hidden border-2 border-orange-300 bg-orange-50 my-3">
                ...
                {third.player?.club && (
                  <p className="text-xs text-slate-500">{third.player.club}</p>
                )}
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-around text-xs">
                  {third.points !== null && (
                    <div>
                      <span className="text-slate-400 block text-[10px]">Puntos</span>
                      <span className="font-mono font-bold text-slate-900">{third.points} pts</span>
                    </div>
                  )}
                  {third.rating_performance && (
                    <div>
                      <span className="text-slate-400 block text-[10px]">Performance</span>
                      <span className="font-mono font-bold text-emerald-600">{third.rating_performance}</span>
                    </div>
                  )}
                </div>

                {third.prize_won && (
                  <p className="mt-3 text-xs font-bold text-orange-800 bg-orange-50 py-1.5 px-3 rounded-lg border border-orange-200">
                    {third.prize_won}
                  </p>
                )}
              </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-xs">
                          {cat ? (
                            <span className="inline-flex items-center px-2 py-0.5 rounded-md bg-blue-50 border border-blue-200 text-[#1D64F2] font-semibold">
                              {cat.name}
                            </span>
                          ) : (
                        ...
                        <td className="px-6 py-4 text-center hidden md:table-cell font-mono text-emerald-600 text-xs">
                          {item.rating_performance ? item.rating_performance : "—"}
                        </td>
                        <td className="px-6 py-4 hidden lg:table-cell text-xs text-slate-700">
                          {item.prize_won ? (
