...
export async function SponsorsSection() {
  ...
  return (
    <section
      className="py-16 sm:py-20 bg-[#0B0F19] border-t border-[#2B5B84]/40"
        ...
        {sortedSponsors.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 items-center">
            {sortedSponsors.map((sponsor) => {
              const cardContent = (
                <div className="h-28 rounded-2xl border border-[#2B5B84]/60 bg-[#132238]/60 p-4 flex flex-col items-center justify-center text-center transition-all duration-300 hover:border-[#5FA8D3] hover:bg-[#132238] hover:shadow-lg hover:shadow-[#5FA8D3]/5 group">
                  {sponsor.logo_url ? (
                    ...
                    <span className="text-sm font-bold text-[#F0F4F8] line-clamp-2">
                      {sponsor.name}
                    </span>
                  )}
                  <span className="text-[10px] uppercase tracking-widest text-[#5FA8D3] font-bold mt-2 opacity-75 group-hover:opacity-100">
              ...
              return sponsor.website_url ? (
                <a
                  ...
                  className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5FA8D3] rounded-2xl"
          ...
          <div className="rounded-2xl border border-[#2B5B84] bg-[#132238] p-8 sm:p-10 text-center shadow-lg max-w-2xl mx-auto">
            <h3 className="text-lg font-bold text-[#F0F4F8]">
              ¿Te gustaría patrocinar el ajedrez dominicano?
            </h3>
            <p className="mt-2 text-xs sm:text-sm text-[#94A3B8] max-w-md mx-auto">
              Únete como marca aliada oficial en nuestros próximos torneos nacionales y regionales.
            </p>
            <div className="mt-6">
              <Link
                href="/contacto"
                className="inline-flex items-center gap-1.5 text-xs sm:text-sm font-bold text-[#5FA8D3] hover:underline"
              >
